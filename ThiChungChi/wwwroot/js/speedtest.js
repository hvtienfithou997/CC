...
function restart() {
    ...
    $.ajax({
        type: "POST",
        url: "/typing/get_words",
        data:
            "de_thi=" + de_thi + "&cuoc_thi=" + cuoc_thi,
        cache: false,
        success: function (data) {
            if (data.data == null) {
                $.notify("Bạn đã hết lượt thi ở cuộc thi này!", "error");
                return;
            }

            max_time = data.data.time;
            countdown = data.data.time;
            $("#timer").text(max_time + "s");
            setTimeout(function () {
                $("#ajax-load").css("display", "none");
                $("#reload-box").css("display", "block");
                $("#wordlist").text(data.data.word);
